...
PetiteVue.createApp({
  ...
  update() {
    ...
    this.percentCompleted = Math.trunc(
      100 -
        (this.minutesLeft /
          this.currentPeriod.end.diff(this.currentPeriod.start, "minutes")) *
          100
    )
    this.percentCompletedText = translateWithInsert(
      "PERCENT_COMPLETED",
    ...
    document.title =
      (this.minutesLeft >= 60
        ? Math.trunc(this.minutesLeft / 60) + "hr. "
        : "") +
      (this.minutesLeft % 60) +
      "min. | LCHS Go"
...
function changeSetting(setting, value) {
  ...
  if (setting == "themeAnimationIntensity")
    rootStyle.setProperty("--animated-background-intensity", value + "deg")
  if (setting == "notificationToggle" && value) {
    if (!("Notification" in window)) {
      ...
      const notification = new Notification("LCHS Go", {
        body: translate("NOTIFY_ENABLED"),
        icon: "/faviconLarge.png",
      })
      // …
...
function themeChange() {
  if (Date.now() - prevTime < 100) {
    ...
    document.getElementById("background").style.backgroundImage =
      "linear-gradient(0,rgba(255, 0, 0, 1) 0%,rgba(255, 154, 0, 1) 10%,rgba(208, 222, 33, 1) 20%,rgba(79, 220, 74, 1) 30%,rgba(63, 218, 216, 1) 40%,rgba(47, 201, 226, 1) 50%,rgba(28, 127, 238, 1) 60%,rgba(95, 21, 242, 1) 70%,rgba(186, 12, 248, 1) 80%,rgba(251, 7, 217, 1) 90%,rgba(255, 0, 0, 1) 100%)"
  }
  prevTime = Date.now()
}
...
export function sendNotification(period, timeLeft) {
  if (settings.notificationToggle && !notified) {
    ...
    if (
      nextPeriod &&
      !nextPeriod.passing &&
      nextPeriod.isVisible() &&
      timeLeft == parseInt(settings.notificationStart)
    ) {
      // period start notif
      const notification = new Notification("LCHS Go", {
        body:
          ...
          translateWithInsert(
            "NOTIFY_START",
            translate(settings.notificationStart)
          ),
        icon: "/faviconLarge.png",
      })
      notified = true
      ...
      !period.passing &&
      period.isVisible() &&
      timeLeft == parseInt(settings.notificationEnd)
    ) {
      // period end notif
      const notification = new Notification("LCHS Go", {
        body:
          ...
          translateWithInsert(
            "NOTIFY_END",
            translate(settings.notificationEnd)
          ),
        icon: "/faviconLarge.png",
      })
      notified = true
