...
export function getSchedule(date) {
  ...
  if (
    Object.keys(scheduleJSON.overrides.all).includes(date.format("MM/DD/YYYY"))
  ) {
    scheduleType = scheduleJSON.overrides.all[date.format("MM/DD/YYYY")]
  } else if (
    Object.keys(scheduleJSON.overrides[settings.grade]).includes(
    ...
    if (isBreak) scheduleType = isBreak
    if (!isBreak && !isCustomWeek) scheduleType = scheduleJSON.defaults[date.day()]
  ...
  if (
    ...
    switch (settings.grade) {
      ...
      case "GRADE_8":
        Object.keys(
          scheduleJSON.gradeLevels.middleSchool[scheduleType]
        ).forEach((period) => {
          if (previousEnd != undefined) {
            localJSON.push({
              name: "PASSING_BEFORE," + period,
              start: previousEnd,
              end: scheduleJSON.gradeLevels.middleSchool[scheduleType][
                period
              ][0],
              passing: true,
            })
          ...
          localJSON.push({
            name: period,
            start:
              scheduleJSON.gradeLevels.middleSchool[scheduleType][period][0],
          ...
          (period) => {
            if (previousEnd != undefined) {
              localJSON.push({
                name: "PASSING_BEFORE," + period,
                start: previousEnd,
                end: scheduleJSON.gradeLevels.highSchool[scheduleType][
                  period
                ][0],
                passing: true,
              })
            ...
            localJSON.push({
              name: period,
              start:
                scheduleJSON.gradeLevels.highSchool[scheduleType][period][0],
        ...
        Object.keys(
          ...
          if (previousEnd != undefined) {
            localJSON.push({
              name: "PASSING_BEFORE," + period,
              start: previousEnd,
              end: scheduleJSON.gradeLevels.summerSchool912[scheduleType][
                period
              ][0],
              passing: true,
            })
          ...
          localJSON.push({
            name: period,
            start:
              scheduleJSON.gradeLevels.summerSchool912[scheduleType][period][0],
            end: scheduleJSON.gradeLevels.summerSchool912[scheduleType][
              period
            ][1],
            passing: false,
          ...
          previousEnd =
            scheduleJSON.gradeLevels.summerSchool912[scheduleType][period][1]
        })
        break
    }
    if (settings.grade >= 9) {
  ...
  for (var range in scheduleJSON.dateRanges.breaks) {
    if (
      date
        ...
        .add(1, "hour")
        .isBetween(
          dayjs(scheduleJSON.dateRanges.breaks[range][0], "MM/DD/YYYY").startOf(
            "day"
          ),
          dayjs(scheduleJSON.dateRanges.breaks[range][1], "MM/DD/YYYY").endOf(
            "day"
          )
        )
    ) {
  ...
  for (var range in scheduleJSON.dateRanges.customWeeks) {
    if (
      date
        ...
        .isBetween(
          dayjs(
            scheduleJSON.dateRanges.customWeeks[range][0],
            "MM/DD/YYYY"
          ).startOf("day"),
          dayjs(
            scheduleJSON.dateRanges.customWeeks[range][1],
            "MM/DD/YYYY"
          ).endOf("day")
        )
    ) {
